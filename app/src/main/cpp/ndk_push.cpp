//
// Created by TIAN FENG  on 2019/4/29.
//
...
#define LOGE(FORMAT,...) __android_log_print(ANDROID_LOG_ERROR,"tianfeng",FORMAT,##__VA_ARGS__)
    ...
    while(is_pushing){
        ...
        if(packet){
            ...
            if(!i){
                ...
                bool isVideo =  packet->m_packetType==RTMP_PACKET_TYPE_VIDEO;
                LOGE("%s     %d","rtmp send packet",isVideo);
            }
            RTMPPacket_Free(packet);
        }
...
Java_com_demo_avpush_jni_PushNative_startPush(JNIEnv *env, jobject instance, jstring url_) {
    //jobj(PushNative对象)
    ...
    const char *url_cstr = env->GetStringUTFChars(url_, 0);
    ...
    env->ReleaseStringUTFChars(url_,url_cstr);
}

extern "C"
JNIEXPORT void JNICALL
Java_com_demo_avpush_jni_PushNative_stopPush(JNIEnv *env, jobject instance) {
    is_pushing = FALSE;
}

extern "C"
JNIEXPORT void JNICALL
Java_com_demo_avpush_jni_PushNative_release(JNIEnv *env, jobject instance) {
...
extern "C"
JNIEXPORT void JNICALL
Java_com_demo_avpush_jni_PushNative_setVideoOptions(JNIEnv *env, jobject instance, jint width,
...
extern "C"
JNIEXPORT void JNICALL
Java_com_demo_avpush_jni_PushNative_setAudioOptions(JNIEnv *env, jobject instance,
...
extern "C"
JNIEXPORT void JNICALL
Java_com_demo_avpush_jni_PushNative_fireVideo(JNIEnv *env, jobject instance, jbyteArray data_) {
    //视频数据转为YUV420P
    //NV21->YUV420P
...
extern "C"
JNIEXPORT void JNICALL
Java_com_demo_avpush_jni_PushNative_fireAudio(JNIEnv *env, jobject instance, jbyteArray buffer,
                                              jint len) {
    int *pcmbuf;
